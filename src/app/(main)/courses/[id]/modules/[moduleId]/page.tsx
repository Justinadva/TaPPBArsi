...
import { ArrowLeft, Play, CheckCircle, ChevronRight, FileText, MessageSquare } from "lucide-react";

// Dummy Data untuk Player
const COURSE_CONTENT = {
  id: "1",
  title: "Modern Apartment Design",
  modules: [
    { id: "1", title: "1. Pengenalan Konsep Modern", duration: "15:00", type: "video", videoUrl: "https://www.youtube.com/embed/dQw4w9WgXcQ" }, // Ganti dengan ID video asli
    { id: "2", title: "2. Denah & Sirkulasi Ruang", duration: "45:00", type: "video", videoUrl: "https://www.youtube.com/embed/dQw4w9WgXcQ" },
    { id: "3", title: "3. Studi Kasus: Apartemen Jepang", duration: "10:00", type: "reading" },
    { id: "4", title: "4. Teknik Pencahayaan Alami", duration: "60:00", type: "video", videoUrl: "https://www.youtube.com/embed/dQw4w9WgXcQ" },
  ]
};

export default function LessonPage({ params }: { params: { id: string; moduleId: string } }) {
  // Di Next.js 15+, params tidak perlu di-await jika client component, tapi untuk aman kita akses langsung
  const currentModule = COURSE_CONTENT.modules.find(m => m.id === params.moduleId) || COURSE_CONTENT.modules[0];
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 min-h-[85vh] animate-in fade-in duration-500">
      ...
      <div className="flex-1 space-y-6">
        ...
        {/* Breadcrumb */}
        <Link href={`/courses/${params.id}`} className="inline-flex items-center gap-2 text-gray-400 hover:text-[#ff8c42] transition text-sm font-medium">
          <ArrowLeft size={16} />
          Kembali ke Detail
        </Link>
      ...
      <div className="w-full lg:w-[380px] flex-shrink-0">
        <div className="glass-card p-5 rounded-[32px] sticky top-24 flex flex-col h-[calc(100vh-120px)] bg-[#1a1a1a]/80 backdrop-blur-xl border border-white/10">
           ...
           <div className="mb-6 px-2">
             ...
             <p className="text-xs text-gray-400 mt-1">{completed.length} / {COURSE_CONTENT.modules.length} Selesai</p>
             {/* Progress Bar */}
             <div className="w-full h-1.5 bg-gray-700 rounded-full mt-3 overflow-hidden">
                <div 
                    ...
                    style={{ width: `${(completed.length / COURSE_CONTENT.modules.length) * 100}%` }}
                />
             </div>
           </div>
           ...
           <div className="flex-1 overflow-y-auto pr-2 space-y-2 custom-scrollbar">
             {COURSE_CONTENT.modules.map((item) => {
               const isActive = item.id === params.moduleId;
               const isDone = completed.includes(item.id);

               return (
                 <Link 
                    key={item.id} 
                    href={`/courses/${params.id}/modules/${item.id}`}
                 ...
                 >
                    <div className="relative">
                       <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold transition ${
                           ...
                           isDone ? "bg-green-500 text-white" : "bg-gray-800 text-gray-400 group-hover:bg-white/10"
                       }`}>
                           {isDone ? <CheckCircle size={16} /> : item.id}
                       </div>
                       {isActive && (
